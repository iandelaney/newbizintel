...
6. Brand, competitor, and source logos
   ...
   - Trust test: Brand, competitor, and news/source logos resolve without generic fallbacks. Competitor logos must prefer real square marks, favicons, or app icons over wide wordmarks; if no real square mark exists, create a square badge by trimming and centring the real acquired wordmark rather than using a generic text card. A square asset is not enough on its own: the visible logo content must occupy enough of the badge to be legible in the competitor table.
