---
name: newbiz2-intake
description: Settle company identity, website assumption, competitor input, and working folder creation for newbiz2. Use when a run is starting from scratch or when the working brand folder and run-state need to be created safely.
---

# NewBiz2 Intake

Own:

- company name
- website assumption
- competitor input capture
- market and audience notes
- working brand folder creation
- initial `run-state.json`

Use `scripts\intake\init_brand_workspace.ps1` for folder creation.
