...
def output_root(explicit_root: str | None) -> Path:
    ...
    env_root = os.environ.get("NEWBIZINTEL_OUTPUT_ROOT") or os.environ.get("NEWBIZ2_OUTPUT_ROOT")
    if env_root:
...
def apply_tavily_reputation_research(data_path: Path, brand_folder: Path, summary: dict[str, Any]) -> tuple[dict[str, Any], Path | None]:
    ...
    if output_path.exists() and os.getenv("NEWBIZINTEL_REFRESH_TAVILY_REPUTATION_RESEARCH", os.getenv("NEWBIZ2_REFRESH_TAVILY_REPUTATION_RESEARCH", "")) != "1":
...
def audit_presentation_html(brand_folder: Path, data_path: Path) -> dict[str, Any]:
    ...
    if "Generated by newbizintel modular runner" in text or "Generated by newbiz2 modular runner" in text or missing_rich_markers:
...
def task_list_html(brand_folder: Path) -> str:
    ...
    return (
        "<section id=\"newbizintel-workflow-task-list\" class=\"newbizintel-task-list newbiz2-task-list\">"
...
def inject_task_list_into_html(html_path: Path, brand_folder: Path) -> None:
    ...
    pattern = re.compile(r"<section id=\"(?:newbizintel|newbiz2)-workflow-task-list\".*?</section>", re.S)
    original_text = text
...
def find_powershell() -> str | None:
    explicit = os.environ.get("NEWBIZINTEL_PWSH") or os.environ.get("NEWBIZ2_PWSH")
    candidates = [
        explicit,
        shutil.which("pwsh"),
...
def module_render(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        if os.environ.get("NEWBIZINTEL_ALLOW_POWERSHELL_RENDER_FALLBACK", os.environ.get("NEWBIZ2_ALLOW_POWERSHELL_RENDER_FALLBACK", "")) == "1":
            html_path = render_rich_html_with_powershell(data_path, html_path)
        elif os.environ.get("NEWBIZINTEL_ALLOW_SKELETAL_RENDER", os.environ.get("NEWBIZ2_ALLOW_SKELETAL_RENDER", "")) == "1":
...
def prepare_random_vercel_stage(data_path: Path) -> dict[str, Any]:
    ...
    force_new = os.environ.get("NEWBIZINTEL_FORCE_NEW_VERCEL_STAGE", os.environ.get("NEWBIZ2_FORCE_NEW_VERCEL_STAGE", "")) == "1"
